...
# Instantiate the loggers for the resources we need to check
...
def calculate_discount(
    ...
    if not promo:
        promotion_logger.log_resource_not_found(promo_id)
        raise HTTPException(
...
def process_purchase(session: Session, payload: PurchaseCreate) -> Purchase:
    ...
    if payload.customer_id:
        ...
        if not customer:
            customer_logger.log_resource_not_found(payload.customer_id)
            raise HTTPException(
                status_code=status.HTTP_404_NOT_FOUND,
