...
`dependencies=[Depends(...)]` on the route decorator (after `responses`). Role
checks build on the existing JWT helpers (`get_token_payload` /
`get_current_employee` from logout/auth work):
