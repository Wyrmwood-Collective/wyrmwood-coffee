...
Review all autogenerated migrations before committing them. The migration should contain only schema changes related to the feature being developed.

SQLAlchemy model changes and their corresponding migration files must be included in the same pull request.

Do not manually create, remove, or modify schema objects in the shared staging database. Schema changes must be represented by Alembic migrations.
